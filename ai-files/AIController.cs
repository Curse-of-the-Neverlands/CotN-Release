...
public class AIController : MonoBehaviour
{
    ...
    public float timeToRotate = 2;                  //  Wait time when the enemy detects the player without seeing
...
 
    ...
    public float edgeDistance = 0.5f;               //  Max distance to calculate the min and max raycast
 
 
    public Transform[] waypoints;                   //  All patrol waypoints
    int m_CurrentWaypointIndex;                     //  current patrol waypoint
 
    Vector3 playerLastPosition = Vector3.zero;      //  last player position near the enemy
    Vector3 m_PlayerPosition;                       //  last seen player position
 
...
 
    ...
    {
        //   chasing the player
        m_PlayerNear = false;                       //  Set false because the enemy already sees the player
...
 
        ...
        if (navMeshAgent.remainingDistance <= navMeshAgent.stoppingDistance)    //  Controls if the enemy arrives to the player location
        {
...
 
    private void Patroling()
    {
        if (m_PlayerNear)
        {
            //  hearing check for the player, moves to that position
            if (m_TimeToRotate <= 0)
            {
                Move(speedWalk);
            ...
            {
                //  wait before going to the players position
                Stop();
        ...
        {
            m_PlayerNear = false;           //  set false if the player isnt near
...
 
        for (int i = 0; i < playerInRange.Length; i++)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToPlayer) < viewAngle / 2)
            {
                ...
                {
                    m_playerInRange = true;             //  The player has been seen by the enemy and then the enemy starts to chase the player
                    m_IsPatrol = false;                 //  Change the state to chasing 
                }
                else
                {
                    /*
